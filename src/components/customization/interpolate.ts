...
export function interpolate2D(
  ...
  // Find index bounds
  let i = widthVals.findIndex((w) => w > width);
  let j = dropVals.findIndex((d) => d > drop);

  // Handle exact boundary values (e.g., if width is exactly the maximum in widthVals)
  if (i === -1 && width === widthVals[widthVals.length - 1]) {
    i = widthVals.length - 1;
  }
  if (j === -1 && drop === dropVals[dropVals.length - 1]) {
    j = dropVals.length - 1;
  }
