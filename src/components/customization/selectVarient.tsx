            ...
            <RadioGroup className="w-full flex xl:flex-nowrap flex-wrap items-stretch gap-2" value={props.selectedValue || selected} onValueChange={handleSelectionChange}>
                {optionData.values.map((option, index) => (
                    <div key={index} className="w-full max-w-[48%] flex items-center justify-between">
                        ...
                        >
                            <div className={`w-full h-[170px]  ${optionTitle === 'Track Colour' ? 'sm:h-[170px]' :' sm:h-[27.344vw] xl:h-[18.229vw]' }  bg-[--white] rounded-24 outline overflow-hidden transition  ${(props.selectedValue || selected) === option.label ? 'outline-2 outline-[--primary] ring-2 ring-[--primary]' : 'outline-1 outline-[--lightGrey] ring-0'}`}>
                                <img
                                    src={option.image}
